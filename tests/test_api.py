...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocs_autorefs`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in mkdocs_autorefs.__all__ or not hasattr(mkdocs_autorefs, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in mkdocs_autorefs.__all__:
